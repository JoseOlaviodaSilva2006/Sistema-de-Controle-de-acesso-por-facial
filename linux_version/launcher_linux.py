...
# Importando da versão Linux
from facial_auth_linux import (
    Storage, FaceEngine, AuthController, DB_PATH, CASCADE_PATH, 
    MODEL_PATH, REQUIRED_SAMPLES, COLOR_PRIMARY, _open_camera
)

# No Linux, o CustomTkinter às vezes precisa que a janela seja escalada adequadamente
...
class App(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("AURA ACCESS CONTROL - LINUX EDITION")
        # Ajustado para possível resolução padrão de telas/TV Boxes
        self.geometry("1280x720")
        
        # Backend e Configuração
        from facial_auth_linux import load_env, logger
        self.cfg = load_env()
    ...
    def _init_sidebar(self):
        ...
        lbl = ctk.CTkLabel(self.sidebar, text="AURA OS", font=ctk.CTkFont(size=28, weight="bold"), text_color="#40adff")
        lbl.pack(pady=(40, 40))
        ...
        self.admin_info = ctk.CTkLabel(self.sidebar, text="Status: Guest", font=("Ubuntu", 11), text_color="gray")
    ...
    def _init_content_frames(self):
        ...
        ctk.CTkLabel(self.f_dash, text="Monitoramento em Tempo Real (Linux)", font=ctk.CTkFont(size=22, weight="bold")).grid(row=0, column=0, padx=40, pady=(40, 0), sticky="w")
        ...
        self.btn_cam = ctk.CTkButton(ctrls, text="ATIVAR SISTEMA", width=250, height=55, font=("Ubuntu", 14, "bold"), command=self.toggle_cam)
        self.btn_cam.pack(side="left", padx=10)
        self.btn_new = ctk.CTkButton(ctrls, text="NOVO CADASTRO", width=250, height=55, fg_color="#16a34a", font=("Ubuntu", 14, "bold"), command=self._show_enroll)
        ...
        self.log_list = tk.Listbox(self.f_logs, bg="#111", fg="#bbb", font=("Monospace", 11), border=0)
    ...
    def toggle_cam(self):
        if not self.is_cam_on:
            ...
            if not self.cap or not self.cap.isOpened():
                messagebox.showerror("Erro", "Câmera não encontrada ou ocupada. Verifique as permissões de vídeo no Linux (ex: video group).")
                return
            self.is_cam_on = True
            self.is_enrolling = False
    ...
    def _cam_loop(self):
        ...
        while self.is_cam_on and not self.stop_event.is_set():
            ...
            if self.is_enrolling:
                ...
                if rect is not None and face_gray is not None:
                    ...
                    if self.enroll_data['count'] >= REQUIRED_SAMPLES:
                        ...
                        def _finish_training():
                            ...
                            self.after(0, lambda: messagebox.showinfo("Aura Access", "Cadastro Biométrico e Treinamento concluídos."))
                            
                        threading.Thread(target=_finish_training, daemon=True).start()
                        
                from facial_auth_linux import VisualHelper, COLOR_PRIMARY
                ...
                from facial_auth_linux import VisualHelper, COLOR_INFO
            ...
            try:
                ...
                self.after(0, lambda i=ctk_img: self.v_label.configure(image=i, text=""))
            except Exception as e:
                break
            time.sleep(0.01)
...
class GalleryWindow(ctk.CTkToplevel):
    def __init__(self, parent, uid, name):
        ...
        ctk.CTkLabel(self, text=f"Banco de Fotos: {name}", font=("Ubuntu", 16, "bold")).grid(row=0, column=0, pady=20)
    ...
    def _load_samples(self):
        ...
        for i, f in enumerate(files):
            ...
            ctk.CTkLabel(frame, text=f.name, font=("Monospace", 10)).pack(side="left", padx=10)
