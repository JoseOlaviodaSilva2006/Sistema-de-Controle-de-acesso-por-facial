...
logger = logging.getLogger("AuraAuthLinux")
...
Path("data/denied").mkdir(parents=True, exist_ok=True)
...
class Storage:
    ...
    def _hash(self, password: str, salt: Optional[str] = None) -> Tuple[str, str]:
        if salt is None:
            ...
            except ValueError: salt_bytes = os.urandom(16)
...
class FaceEngine:
    def __init__(self, cascade_path: Path, model_path: Path, storage: Storage):
        ...
        if self.cascade.empty():
            logger.error(f"Falha ao carregar o classificador Haar Cascade em {cascade_path}")
...
def _open_camera(index=0):
    """
    Motor de câmera otimizado para Linux (Video4Linux2), ideal para SBCs e TV Boxes com Armbian.
    """
    logger.info(f"Tentando inicializar câmera {index} com V4L2...")
    cap = cv2.VideoCapture(index, cv2.CAP_V4L2)
    if not cap.isOpened():
        cap.release()
        logger.warning("Falha com V4L2. Tentando método padrão...")
        cap = cv2.VideoCapture(index)
    
    if not cap.isOpened():
        cap.release()
        logger.warning(f"Falha na câmera {index}. Tentando index 1...")
    ...
    if cap.isOpened():
        ...
        # Define codec MJPG que performa muito melhor em SBCs/Armbian via USB
        cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*'MJPG'))
        cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)
        cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)
        # Pode forçar framerate se necessário: cap.set(cv2.CAP_PROP_FPS, 30)
    else:
